...
| `mesh_export.py` | Marching Cubes on a `grid_resolution³` grid at `iso_threshold=0.5`, exported via trimesh. The occupancy grid is truncated to the intersection of all view frustums first (paper Sec. 3.4), so stray geometry cannot cast shadows outside the targets. |
